//
//  RaPlayerDownloadActionWorker.swift
//  RaVideoPlayer
//
...
import Foundation
public protocol RaPlayerDownloadActionWorkerDelegate: NSObjectProtocol {
    func downloadActionWorker(_ actionWorker: RaPlayerDownloadActionWorker, didReceive response: URLResponse)
    func downloadActionWorker(_ actionWorker: RaPlayerDownloadActionWorker, didReceive data: Data, isLocal: Bool)
    func downloadActionWorker(_ actionWorker: RaPlayerDownloadActionWorker, didFinishWithError error: Error?)
}
open class RaPlayerDownloadActionWorker: NSObject{
    open fileprivate(set) var actions = [RaPlayerCacheAction]()
    open fileprivate(set) var url: URL
    open fileprivate(set) var cacheMediaWorker: RaPlayerCacheMediaWorker
    
    open fileprivate(set) var session: URLSession?
    open fileprivate(set) var task: URLSessionDataTask?
    open fileprivate(set) var downloadURLSessionManager: RaPlayerDownloadURLSessionManager?
    open fileprivate(set) var startOffset: Int = 0
    
    open weak var delegate: RaPlayerDownloadActionWorkerDelegate?
    
    fileprivate var isCancelled: Bool = false
    fileprivate var notifyTime = 0.0
    
    
    public init(actions: [RaPlayerCacheAction], url: URL, cacheMediaWorker: RaPlayerCacheMediaWorker) {
        self.actions = actions
        self.cacheMediaWorker = cacheMediaWorker
        self.url = url
        super.init()
        downloadURLSessionManager = RaPlayerDownloadURLSessionManager(delegate: self)
        let sessionConfiguration = URLSessionConfiguration.default
        let session = URLSession(configuration: sessionConfiguration, delegate: downloadURLSessionManager, delegateQueue: RaPlayerCacheSession.shared.downloadQueue)
        ...
        if let action = firstAction {
            if action.type == .local { // local
                if let data = cacheMediaWorker.cache(forRange: action.range) {
                    ...
                    let nsError = NSError(domain: "com.Raplayer.downloadActionWorker", code: -1, userInfo: [NSLocalizedDescriptionKey: "Read cache data failed."])
    ...
    internal func notify(downloadProgressWithFlush isFlush: Bool, isFinished: Bool) {
        ...
        let interval = RaPlayerCacheManager.mediaCacheNotifyInterval
        if notifyTime < currentTime - interval || isFlush {
            if let configuration = cacheMediaWorker.cacheConfiguration?.copy() {
                let configurationKey = RaPlayerCacheManager.RaPlayerCacheConfigurationKey
                let userInfo = [configurationKey: configuration]
                NotificationCenter.default.post(name: .RaPlayerCacheManagerDidUpdateCache, object: self, userInfo: userInfo)
                
                if isFinished && (configuration as! RaPlayerCacheMediaConfiguration).progress >= 1.0 {
    ...
    internal func notify(downloadFinishedWithError error: Error?) {
        if let configuration = cacheMediaWorker.cacheConfiguration?.copy() {
            let configurationKey = RaPlayerCacheManager.RaPlayerCacheConfigurationKey
            let finishedErrorKey = RaPlayerCacheManager.RaPlayerCacheErrorKey
            var userInfo: [String : Any] = [configurationKey: configuration]
            if let er = error {
                userInfo[finishedErrorKey] = er
            }
            NotificationCenter.default.post(name: .RaPlayerCacheManagerDidFinishCache, object: self, userInfo: userInfo)
        }
        
    }
}

// MARK: - RaPlayerDownloadeURLSessionManagerDelegate
extension RaPlayerDownloadActionWorker: RaPlayerDownloadeURLSessionManagerDelegate {
    ...
    public func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        cacheMediaWorker.cache(data, forRange: range) { [weak self] (isCache) in
            ...
            if (!isCache) {
                let nsError = NSError(domain: "com.Raplayer.downloadActionWorker", code: -2, userInfo: [NSLocalizedDescriptionKey: "Write cache data failed."])
