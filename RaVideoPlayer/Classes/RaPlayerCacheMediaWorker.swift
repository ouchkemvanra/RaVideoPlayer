...
import Foundation
import UIKit
open class RaPlayerCacheMediaWorker: NSObject {
    open fileprivate(set) var cacheConfiguration: RaPlayerCacheMediaConfiguration?
    ...
    fileprivate let writeFileQueue = DispatchQueue(label: "com.Raplayer.cacheWriteFileQueue")
    ...
    public init(url: URL) {
        let path = RaPlayerCacheManager.cacheFilePath(for: url)
        filePath = path
        ...
        if err == nil {
            ...
            if err == nil {
                ...
                do {
                    ...
                    cacheConfiguration = RaPlayerCacheMediaConfiguration.configuration(filePath: path)
    ...
    open func cachedDataActions(forRange range:NSRange) -> Array<RaPlayerCacheAction> {
        var actions = [RaPlayerCacheAction]()
            ...
            for (_, value) in cachedSegments.enumerated() {
                ...
                if intersctionRange.length > 0 {
                    ...
                    for i in 0...package {
                        ...
                        let action = RaPlayerCacheAction(type: .local, range: ra)
        ...
        if actions.count == 0 {
            let action = RaPlayerCacheAction(type: .remote, range: range)
            actions.append(action)
        } else {
            var localRemoteActions = [RaPlayerCacheAction]()
            for (index, value) in actions.enumerated() {
                ...
                if index == 0 {
                    if range.location < actionRange.location {
                        ...
                        let action = RaPlayerCacheAction(type: .remote, range: ra)
    ...
    open func set(cacheMedia: RaPlayerCacheMedia) -> Bool {
